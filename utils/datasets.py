...
def pad_to_square(img, pad_value):
    ...
    # Determine padding
    pad = (0, 0, pad1, pad2) if h <= w else (pad1, pad2, 0, 0) # (x1, x2, y1, y2)
    # Add padding
...
class CocoDetection(Dataset):
    ...
    def __init__(self, list_path, img_size=416, augment=True, multiscale=True, normalized_labels=True):
        ...
        self.label_files = [
                    os.path.join('data/cola/labels_txt', path.replace(".png", ".txt").replace(".jpg", ".txt"))
                    for path in self.img_files
                ]
        self.img_size = img_size
        self.max_objects = 100
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join('data/cola/images', self.img_files[index % len(self.img_files)].rstrip())
